...
export default function SignUpGuest() {
  ...
  // const [phoneNumber, setPhoneNumber] = useState<string>(() => getStoredValue("phoneNumber"));
  // const [password, setPassword] = useState<string>(() => getStoredValue("password"));

  const [phoneNumber, setPhoneNumber]:any = useState([]);
  const [password, setPassword]:any = useState([]);
  const [send, setSend]: any = useState([]);
  ...
  useEffect(() => {
      const storedPhoneNumber:any = localStorage.getItem("phoneNumber");
      if (storedPhoneNumber) {
        setPhoneNumber(storedPhoneNumber.toString());
      }
  }, [phoneNumber]);
  ...
  useEffect(() => {
      ...
      if (storedPassword) {
        setPassword(storedPassword.toString());
      }
  }, [password]);

  useEffect(() => {
      const storedSend:any = localStorage.getItem("send");
      if (storedSend) {
        setSend(storedSend.toString());
      }
  }, [send]);

  const sendMessage = (phoneNumber: string, password: string) => {
    const sender = '6281212499837'; // No.Hp Pengirim
    const apiKey = 'jTYnHBlDlPrzM0EFEidotthE2Xnv18'; // API KEY Anda
    const url = 'https://server.wa-bisnis.com/send-message'; // URL API
    const receiver = phoneNumber; // No.HP Penerima
    const pesan = `Welcome to Realta Hotels. Your login access with password is :\n${password.toString()}`; // Pesan yang dikirim
  ...
  if ([phoneNumber] && password && send) {
    sendMessage(phoneNumber, password);
  }
